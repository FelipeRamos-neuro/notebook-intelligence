...
- **Real progress feedback** during long Claude turns. An elapsed-time counter, a heartbeat-driven pulse with a "may be slow" copy flip after 30 seconds, and a persistent status card for every tool the agent runs (kind icon, live in-progress / done / failed state, and an inline diff for edits) so the sidebar reflects what the agent is doing. A run of consecutive calls collapses into one expandable group.
